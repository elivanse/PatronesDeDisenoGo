...
func TestGetInstance(t *testing.T) {
	...
	if currentCount != 1 {
		t.Errorf("After callign for the first time to count, the count must be  1 but is %d\n", currentCount)
	}
	...
	if currentCount != 2 {
		t.Errorf(" After calling 'addone'  using the second counter, the cuurrent count must be 2  but was %d\n", currentCount)
	}
}
